...
<div class="form-group">
  <label class="col-sm-3 control-label">Prefijo <span class="obligatorio">(*)</span> :</label>
  <div class="col-sm-6">
      <input  type="text"
              id="aux01" name='aux01' 
              value="@if(isset($categoria)){{old('aux01' ,$categoria->aux01)}}@else{{old('aux01')}}@endif"
              value="{{ old('aux01') }}" 
              placeholder="Prefijo"
              required = ""
              autocomplete="off" class="form-control input-sm" data-aw="5"
              />
      @include('error.erroresvalidate', [ 'id' => $errors->has('aux01')  , 
                                          'error' => $errors->first('aux01', ':message') , 
                                          'data' => '5'])
  </div>
</div>
