  ...
  <tbody>
    @foreach($listadatos as $index => $item)
      <tr data_certificado_id = "{{$item->id}}">
        ...
        <td class="cell-detail sorting_1" style="position: relative;"> 
          <span><b>GASTO : </b>{{$item->tipo_gasto_nombre}}</span> 
          <span><b>COMPRA : </b>{{$item->tipo_compra_nombre}}</span> 
        ...
        <td>
          @if($item->estado_id=='ESEG00000001')
            <span class="badge badge-light">{{$item->estado_nombre}}</span>
          @else
            @if($item->estado_id=='ESEG00000002')
        ...
        <td class="rigth">
          <div class="btn-group btn-hspace">
            ...
            <ul role="menu" class="dropdown-menu pull-right">              
              ...
              <li>
                <a href="{{ url('/descargar-archivo-egreso/'.$idopcion.'/'.Hashids::encode(substr($item->archivo_id, -8))) }}">
                  Descargar
                </a>  
              </li>              
              @endif
              @if($item->estado_id == 'ESEG00000001') 
                <li>
                  <a href="{{ url('/modificar-egreso/'.$idopcion.'/'.Hashids::encode(substr($item->id, -8))) }}">
                    Modificar
                  </a>  
                </li>
              @endif
              @if($item->estado_id == 'ESEG00000001') 
                <li>
                  <a href="{{ url('/emitir-egreso/'.$idopcion.'/'.Hashids::encode(substr($item->id, -8))) }}">
                    Emitir
                  </a>  
                </li>
              @endif
